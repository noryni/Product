...
function Library:save_flags()
    ...
    if fs_supported then
        writefile(`Noryn/{game.GameId}.lua`, flags)
    else
...
function Library:load_flags()
    if fs_supported then
        if not isfile(`Noryn/{game.GameId}.lua`) then 
            Library:save_flags() 
            return 
        end
        local flags = readfile(`Noryn/{game.GameId}.lua`)
        if not flags then 
            Library:save_flags() 
            return 
        end
...
print('UI - ©noryni (Github)') --// [⚠️ DO NOT DELETE THIS!]
