...
if not Current_Config then
    local Current_Config = {
        ...
        Macro_Emote_Enabled = true, --// [For Da-Hood Games]
...
if not Settings.Noryn_DLL_Loaded then Settings.Noryn_DLL_Loaded = true
...
╚═╝░░╚══╝░╚════╝░╚═╝░░╚═╝░░░╚═╝░░░╚═╝░░╚══╝
    ...
    warn('• @ Noryn DLL | © COPYRIGHT 2026')
    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.F9, false, game)
else
    local Previous_Config = Settings._Noryn_LastConfig or {}
    local Changed = false
...
Settings._Noryn_LastConfig = table.clone(Current_Config)
...
if Settings.Aim_Helper_Fov then
    pcall(function()
        Settings.Aim_Helper_Fov:Remove()
    end)
...
Settings.Resolver_Strength = math.clamp(Current_Config.Resolver_Strength or 0.85, 0, 1)
...
Settings.Target_Scanner_Running = false
Settings.Target_Scanner_Connection = nil

Settings.Hitbox_Updater_Running = false

--// [INTERVALS]

Settings.Target_Scan_Interval = 0.03 --// [Lower value if you have good PC | 0.03 = 30 ms]
Settings.Hitbox_Update_Interval = 0.1 --// [Lower value if you have good PC]
...
Settings.Velocity_Spoofer_Interval = 0
Settings.X_Velocity = tonumber(Current_Config.X_Velocity) or 0
Settings.Y_Velocity = tonumber(Current_Config.Y_Velocity) or 500
Settings.Z_Velocity = tonumber(Current_Config.Z_Velocity) or 0

Settings.Trigger_Bot_Accuracy_Chance = Parse_Chance(Current_Config.Trigger_Bot_Accuracy or 70)
Settings.Trigger_Bot_Detection_Delay = math.max(Current_Config.Trigger_Bot_Detection_Delay or 0.010, 0)

Settings.Safety_Fov_Enabled = Current_Config.Safety_Fov_Enabled
Settings.Fov_Radius = math.clamp(Current_Config.Fov_Radius or 60, 30, 200)
...
Settings.Hitbox_Mode = tostring(Current_Config.Hitbox_Mode or 'Global')
...
Settings.Fps_Value = tonumber(Current_Config.Fps_Value) or 165
...
if Set_Frame_Cap then
    local function Update_Frame_Cap()
        if Settings.Fps_Stability_Enabled then
            Set_Frame_Cap(math.max(tonumber(Settings.Fps_Value) or 165, 1))
        else
            Set_Frame_Cap(360)
        end
    end
...
local function Create_Fov()
    ...
    Circle.Radius = math.clamp(Settings.Fov_Radius or 60, 30, 200)
...
local function Get_Resolved_Velocity(Target, Delta_Time)
    ...
    if not Entry then
        Entry = {
            ...
            Time = Current_Time,
            Velocity = Target.AssemblyLinearVelocity
        }
    ...
    local Smoothed_Velocity = Previous_Velocity:Lerp(Estimated_Velocity, Strength)
...
local function Get_Closest_Target()
    ...
    local Closest_Distance = Use_Fov_Limit and math.clamp(Settings.Fov_Radius or 60, 30, 200) or math.huge
    ...
    while Settings.Target_Scanner_Running do
        ...
        if Settings.Aim_Helper_Enabled and Camera then
            if UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
                Settings.Cached_Target = Get_Closest_Target()
            else
                Settings.Cached_Target = nil
            end
        else
        ...
        task.wait(Settings.Target_Scan_Interval or 0.05)
    end
end)
...
local function Is_Mouse_On_Target(Target)
    ...
    if not Mouse_Target:IsDescendantOf(Target.Parent) then return false end
...
local function Trigger_Bot_Click()
    ...
    task.spawn(function()
        ...
        local Delay = math.max(Settings.Trigger_Bot_Detection_Delay or 0.010, 0)
        ...
        if not Is_Mouse_On_Target(Target) then Settings.Trigger_Bot_Activated = false return end
...
task.spawn(function()
    while Settings.Hitbox_Updater_Running do
        if Settings.Hitbox_Enabled then
            ...
            local Mode = string.lower(tostring(Settings.Hitbox_Mode or 'Global'))
...
Settings.Aim_Helper_Connection = RunService.RenderStepped:Connect(function(Delta_Time)
    ...
    if Should_Show_Fov then if not Fov then Create_Fov() end else if Fov then Remove_Fov() end end
    ...
    if Fov then
        ...
        Settings.Fov_Radius = math.clamp(Settings.Fov_Radius or Current_Config.Fov_Radius or 60, 30, 200)
    ...
    local Target = Settings.Cached_Target or Get_Closest_Target()
    ...
    local Use_Fov_Limit = Settings.Fov_Enabled or Settings.Safety_Fov_Enabled
    local Max_Fov = Use_Fov_Limit and math.clamp(Settings.Fov_Radius or 60, 30, 200) or math.huge
    ...
    if Settings.Trigger_Bot
        ...
        and Is_Aim_Active()
        and Is_Mouse_On_Target(Target) then
        Trigger_Bot_Click()
    end

    if not Settings.Aim_Accuracy then
        return
    end
    ...
    local Predicted_Position = Target.Position + (Resolved_Velocity * Settings.Prediction) + Vector3.new(X_Offset, Y_Offset, 0)
...
if Settings.Join_Discord then Run(Invite)
    Log(`[Notify] - @{tostring(LocalPlayer.Name)} was invited to the official Noryn DLL Discord server. 💜`)
end
